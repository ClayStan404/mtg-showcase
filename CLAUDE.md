...
- **`image_cdn`**: set in `site_config.json` (`"scryfall"` | `"mtgch"`). **Production uses explicit `scryfall`** after A/B (mtgch CDN felt slower). Code fallback `DEFAULT_IMAGE_CDN` is `mtgch` only when the key is missing. Preferred CDN miss → other host + `image_cdn_attempted` (sticky: no auto re-check when preferred later gains art; flip config or clear marker to force). Single mtgch `image_url` makes small/normal/large identical.
