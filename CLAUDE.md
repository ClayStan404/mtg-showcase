...
- **mtgch API**: `https://mtgch.com/api/v1/card/{set}/{number}/` - Chinese names + optional image URLs (`images.mtgch.com`). Snapshots store **hotlink URLs only** (no image proxy).
- **`image_cdn`**: set in `site_config.json` (`"scryfall"` | `"mtgch"`). **Production uses explicit `scryfall`** after A/B. Code fallback `DEFAULT_IMAGE_CDN` is `mtgch` only when the key is missing. Orthogonal sticky fields: `image_cdn_attempted` (host tried) + `zhs_art_attempted` (Chinese face tried). `image_lang` is face language of the chosen art (set by resolve path, not URL heuristics — Scryfall URLs have no `/zhs/`). For `lang=zhs`, Chinese face wins over CDN host flip; once `zhs_art_attempted` and still English face, no auto re-try when mtgch later gains art (flip config / clear flags to force). Frontend shows `无中文印刷图` when `lang=zhs` but `image_lang` is not Chinese.
