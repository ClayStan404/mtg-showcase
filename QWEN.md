...
- **Heartbeat workflow** (`heartbeat.yml`): GitHub-hosted runner every **2h** (schedule only; no `workflow_run`) checks whether auto-update has succeeded within 2h; if stale, opens/comments an issue; auto-closes on recovery (may lag up to ~2h).
