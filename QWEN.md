...
- **Data**: Storage `site-data/cards.json` + `wants.json` (primary); lazily loaded `assets/*-data.js` fallback; same shape as `data/*.json` build outputs
...
| `.github/workflows/` | `ci.yml` (PR/push checks), `auto-update.yml` (deploy), and monitoring/backup workflows |
...
| `assets/` | Frontend source including `mtg-ui.js`, `app.js`, `inventory-parser.js`, generated `site-config.js`, styles, icons, and vendored Supabase; `*-data.js` fallbacks are generated and ignored |
| `tests/`, `tests-js/` | Python pytest regressions and Node built-in test-runner coverage |
| `supabase/` | Local CLI config, migrations, Edge Function source, and deployment notes |
...
- The browser application remains framework-free; npm is used only for tests, Edge Function bundling checks, and reproducible Supabase client vendoring.
...
- Card data is fetched from Storage/local JSON first. Generated `assets/cards-data.js` and `assets/wants-data.js` are injected lazily only when those requests fail.
...
- CSP: main site `script-src 'self'`; `connect-src` permits only this project's Supabase host; admin also permits mtgch and Scryfall preview APIs. Inline handlers use `addEventListener` (`bindImgErrors`).
...
Site-level config: `title`, `subtitle`, `supabase_url`, `supabase_anon_key` (public). `build_common.py` reads it and generates `assets/site-config.js` so the shell can connect before snapshot data loads.
...
- Source-controlled CLI config and migration reconciliation are under `supabase/`; see `supabase/README.md`.
...
- **Heartbeat workflow** (`heartbeat.yml`): GitHub-hosted runner **hourly** (schedule only; no `workflow_run`) checks auto-update success within **2h** and db-backup success within **36h** (separate issues); auto-closes on recovery (may lag up to ~1h). ~720 hosted min/mo under free private 2000.
...
- Cache busting (`?v=N`): `bump_all_caches` updates static source references, including `site-config.js` and `inventory-parser.js`; generated data fallbacks are lazy and not present in the initial HTML.
- `.gitignore`: `.venv/`, `.cache/`, `node_modules/`, `__pycache__/`, `**/.~*`, `site/`, `.qwen/`, `.claude/`, `inventory/`, `wants/`, `data/cards.json`, `data/wants.json`, `assets/cards-data.js`, `assets/wants-data.js`.
- `auto-update.yml` `paths-ignore` prevents docs/tests-only changes from deploying; the separate CI workflow still validates test changes.
...
- CI on pull requests and `master` pushes runs Ruff, pytest, Node tests, Edge bundling, and vendored-client reproducibility checks; deploy pushes repeat the relevant checks.
