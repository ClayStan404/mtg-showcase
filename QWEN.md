...
- **Heartbeat workflow** (`heartbeat.yml`): GitHub-hosted runner **hourly** (schedule only; no `workflow_run`) checks whether auto-update has succeeded within 2h; if stale, opens/comments an issue; auto-closes on recovery (may lag up to ~1h). ~720 hosted min/mo under free private 2000.
